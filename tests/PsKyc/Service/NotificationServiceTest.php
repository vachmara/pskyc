...
class NotificationServiceTest extends MockeryTestCase
{
    ...
    public function testSendModernThemeEmailWithTemplating()
    {
        \Mail::shouldReceive('Send')
            ->once()
            ->andReturn(true);
        ...
        $reflection = new \ReflectionClass($this->service);
        $method = $reflection->getMethod('sendModernThemeEmail');
        $method->setAccessible(true);
    ...
    public function testSendModernThemeEmailWithoutTemplating()
    {
        ...
        $reflection = new \ReflectionClass($serviceWithoutTemplating);
        $method = $reflection->getMethod('sendModernThemeEmail');
        $method->setAccessible(true);

        $result = $method->invokeArgs($serviceWithoutTemplating, [
            'test_template',
            'Test Subject',
            ['test' => 'data'],
            'test@example.com',
    ...
    public function testSendModernThemeEmailEmptyRecipient()
    {
        $reflection = new \ReflectionClass($this->service);
        $method = $reflection->getMethod('sendModernThemeEmail');
        $method->setAccessible(true);
